...
function renderLocs(locs) {
    ...
    var strHTML = locs.map(loc => {
        ...
        <li class="loc ${className}" data-id="${loc.id}">
            <h4>  
                <span>${loc.name}</span>
                <span class="distance">${gUserPos ? `Distance: ${utilService.getDistance(gUserPos, loc.geo, 'K')} KM` : ''}</span>
                <span title="${loc.rate} stars">${'★'.repeat(loc.rate)}</span>
            </h4>
            <p class="muted smaller loc-address">
                ${loc.geo.address}
            </p>
            <p class="muted">
...
function onUpdateLoc(locId) {
    locService.getById(locId)
        .then(loc => {
            gLocToSave = loc
            document.querySelector('dialog input[type="text"]').value = loc.name
            document.querySelector('dialog input[type="number"]').value = loc.rate
            onOpenLocDialog()
        })
        .catch(err => {
            console.error('OOPs:', err)
...
function displayLoc(loc) {
    ...
    el.querySelector('.loc-distance').innerText = gUserPos ? `Distance: ${utilService.getDistance(gUserPos, loc.geo, 'K')} KM` : ''
...
function onSaveLocDialog() {
    ...
    gLocToSave.rate = rate

    locService.save(gLocToSave)
        .then(savedLoc => {
